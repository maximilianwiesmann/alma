...
LargeInt LargeInt::power(const LargeInt & arg) const 
{
	if (arg._v.size() == 1 and arg._v[0] == 1) return *this;
	...
	{
		c.div2();
		return x.power(c)*x.power(c);
	}
	else
	{
		LargeInt d = c-1;
		d.div2();
		return x.power(d)*x.power(d+LargeInt(1));
	}
}

LargeInt LargeInt::kaba(const LargeInt & arg) const
{

	std::size_t s1 = _v.size()
	std::size_t s2 = arg._v.size()

	if (s == 1) return *this;

	LargeInt x1(0), x2(0), y1(0), y2(0);
	x1._v.pop_back();
	x2._v.pop_back();
	y1._v.pop_back();
	y2._v.pop_back();
	...
	for (auto i : _v)
	{
		if (i < s1) x1.push_back(i);
		else x2.push_back(i);
	}

	for (auto i : arg._v)
	{
		if (i < s2) y1.push_back(i);
		else y2.push_back(i);
	}
}
