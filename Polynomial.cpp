...
const Polynomial Polynomial::operator-(const Polynomial & arg) const            // Subtraktion
{
    Polynomial res;
    res = *this;
    if (_coefficient.size() < arg._coefficient.size())
    {
    ...
    // Nullen werden nicht entfernt, da sont Division nicht funktioniert
...
{
    Polynomial res;
    res._coefficient.resize(_coefficient.size()+arg._coefficient.size()-1, 0);

    for(size_t it1 = 0; it1 < _coefficient.size(); ++it1)
    {
        for(size_t it2 = 0; it2 < arg._coefficient.size(); ++it2)
        {
            res._coefficient[it1+it2] += _coefficient[it1]*arg._coefficient[it2];
        }
    }
    return res;
}
...
const Polynomial Polynomial::operator/(const Polynomial & arg) const            // Division
{
    ...
    for (size_t it = _coefficient.size() - 1; it >= d && y; --it)
    {
        ...
        {
            if(rest._coefficient[it] != 0)
            {
                index = it - d;
                value = rest._coefficient[it] / co;
                result._coefficient[index] += value;
                teiler.SetCoefficient(index, value);
                rest = rest - arg*teiler;
            }
        }
        else
        {
